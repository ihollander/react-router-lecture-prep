...
function StarRatingInput({ value, maxRating, symbol, color, name, onClick }) {
  ...
  const handleClick = e => {
    e.persist()
    const newRating = Math.ceil((e.nativeEvent.offsetX / e.target.offsetWidth) * maxRating)
    onClick({ ...e, target: { ...e.target, value: newRating, name: name } })
  }

  return (
    <div onClick={handleClick} className="ratings" style={{ cursor: "pointer" }}>
